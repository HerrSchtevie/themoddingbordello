...
<p class="stat-line"><strong>2</strong> PROFILES · <strong>170</strong> GB DOWNLOAD · <strong>268</strong> GB INSTALLED · FULLY AUTOMATED VIA WABBAJACK</p>
...
<tr>
<td align="center">
<a href="https://www.nexusmods.com/skyrimspecialedition/mods/173492"><strong>View on Nexus Mods</strong></a><br>
View the Modlist
</td>
...
<td align="center">
<a href="/modlists/vov/load-order"><strong>Load Order</strong></a><br>
...
Visions of Vaermina (VOV) is a Skyrim modlist designed for mature audiences. It includes NSFW content, OStim integration, and other adult-oriented modifications. Ensure you are legally allowed to access such content in your region before proceeding.
...
- **Two Unique Profiles Included:**  
  VOV offers two distinct profiles tailored to your preferred experience:  
...
- **NSFW & OStim Integration:**  
  VOV integrates OStim Standalone, OBody, HIMBO, and 3BA with immersive balance. All adult content is carefully implemented — no defeat mods, no excessive scripts, and no forced interactions.
...
| **Disk Space**| ~170GB Download / ~268GB Installed / ~438GB Total       | ~170GB Download / ~268GB Installed / ~438GB Total    |
...
3. **Create Folders and Defender Exceptions**
   ...
   - **Important:** do not bury your `\modlists` folder deep in your directory system. If the absolute path to your list install is much longer than `D:\modlists\VOV\` you may encounter cryptogenic errors resulting from overlong file paths.
...
4. **Launch Wabbajack**
   ...
   - In the **upper-left corner**, enable both:
     ...
     - **Show NSFW Lists**
   - Use the search bar to find **Visions of Vaermina (VOV)**.
...
5. **Select the Modlist**
   ...
   - Choose:
     - A **Download Location** – where Wabbajack will store mod archives. Select the `\downloaded mods` directory from step 3.  
       Requires approximately **170 GB of free space**.
     - An **Installation Location** – where the modlist will be fully installed and run from. Create a `\VOV` directory in the `\modlists` folder from step 3.  
       Requires approximately **268 GB of free space**.  
...
- Navigate to your **Visions of Vaermina installation folder**.
...
Visions of Vaermina includes **two distinct profiles**, each preconfigured with its own settings, tool outputs, and game experience.  
...
1. Delete the ENB Cache in your overwrite folder (e.g. `D:\modlists\VOV\overwrite`).
...
### 3. Configure VOV to Your Preferences

Before diving into your playthrough, take a moment to configure the list to match your setup. Whether you're using ultrawide resolutions or a controller, Visions of Vaermina is built to adapt — but it expects you to pick your setup and stick with it.

---

### Widescreen & Resolution Support

Visions of Vaermina includes full widescreen support with UI presets for all major aspect ratios. These options are found near the **bottom of MO2** under the **`Resolution Presets`** separator (highlighted in blue).

- **Always enabled:**
  - `VOV - Core UI`

- **Choose one (and only one) based on your monitor:**
  - `VOV - 21x9 UI Patch` for ultrawide (21:9)
  - `VOV - 32x9 UI Patch` for super ultrawide (32:9)

![image](https://github.com/user-attachments/assets/66d96c2b-7805-4846-9da7-43a19d7a96d9)
...
Controller support is fully integrated into VOV, but all related mods are **disabled by default** so players can opt in manually. To enable controller support:
...
2. **Enable all of the following mods**:
   ...
   ![image](https://github.com/user-attachments/assets/5f13b251-2b08-4afc-b3a5-9b34d761fe39)
...
Visions of Vaermina ships an optional set of skimpier vanilla armor and clothing replacers. Everything in it is **disabled by default** and safe to leave that way.
...
![image](/assets/readmes/vov-spicier.png)
...
- Ensure **`Visions of Vaermina`** is selected as the modlist in the **top-right executable dropdown** in MO2.
- Click **Run** to start the game.

> **Important Warning: Never Click "Unlock" in MO2**
>
> Do **not** click **"Unlock"** in Mod Organizer 2 while Visions of Vaermina is launching or running.
>
...
- The list configures its own MCMs for you. There is nothing to run and no recording to sit through; it settles during the first minute or two, and the popup **"Visions of Vaermina MCM configuration complete."** confirms when it is done.
...
> MCM Recorder is still installed, but nothing plays back on its own any more. It stays in the list as an optional tool. Replaying the `Scrolls of Dreams` recording puts the MCMs back to the settings the list ships with, and if you use a tenkeyless keyboard you can run `Scrolls of Dreams - OStim TKL` once per new game for the alternate keybinds.
...
- To address this, we include Skyrim Save System Overhaul 3 (SSSO3). This mod ensures that saves are only written when the engine can safely create a clean file, greatly reducing the chance of progressive corruption. Refer to the [JOJ Gameplay Guide](/modlists/joj/gameplay-guide#skyrim-save-system-overhaul-3) for configuration instructions.
...
- Loading during play is even riskier than saving. When you reload mid-session, leftover data from the current session mixes with the new one; then, the next time you save, that bad data is baked into your save file from then on.
  ...
  - Again, this guidance applies to all Skyrim configurations, not just VOV.
...
VOV does not have its own gameplay guide. It is a visuals-only list, and the JOJ guide covers the shared systems where they overlap.

- [JOJ Gameplay Guide](/modlists/joj/gameplay-guide): key gameplay systems, progression, mechanics, and builds.
...
By choosing to modify or tweak **Visions of Vaermina (VOV)** in any way — whether by adding mods, changing load order, editing settings, or rebuilding components — you are doing so at your own risk.
...
Thank you for respecting the work that went into VOV, and for being part of the community!
...
- [Visions of Vaermina on Nexus Mods](https://www.nexusmods.com/skyrimspecialedition/mods/173492)
...
Thank you for taking the time to install and experience **Visions of Vaermina**. This project is a passion-driven labor of love built on the shoulders of an incredible modding community.

Whether you're here for the lore, the visuals, the chaos, or the adult content — I truly appreciate you choosing VOV as your journey through Skyrim.
